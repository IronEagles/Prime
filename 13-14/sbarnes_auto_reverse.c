...
task main()
{
  ...
  while(SensorValue[irsensor] > 6 || SensorValue[irsensor] ==0){
	...
	while(nMotorEncoder[RightDrive] > (-1*4*360*5.6)){
		moveForward(SPEED);
		nxtDisplayCenteredTextLine(3, "Encoder: %d", nMotorEncoder[RightDrive]);
	}
	...
	while(nMotorEncoder[RightDrive] > -1*4*360*4)
	{
		motor[RightDrive] = -60;
		motor[LeftDrive] = -35;
	}
	...
	while(true)
	{
		nxtDisplayCenteredTextLine(3, "Heading: %d", currHeading);
		wait1Msec(10);
		if (currHeading >= 75 && currHeading < 95) break;
	}
	halt();
	resetEncoders();
